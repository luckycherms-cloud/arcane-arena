...
///
/// KNOWN GAP (issue #426 — real root cause): the engine surfaces the
/// multi-target `TriggerTargetSelection` correctly (see the test above), but
/// `Discard`'s resolver (`game/effects/discard.rs`) resolves for only the
/// FIRST `TargetRef::Player` in `ability.targets` via
/// `resolve_player_for_context_ref`'s `find_map`. Multi-target-over-`Player`
/// effects need a per-targeted-player resolution fan-out at
/// `resolve_ability_chain` (mirroring the `player_scope` fan-out). That
/// fan-out is not yet implemented, so a 2-player selection only discards the
/// first player. `#[ignore]` until the fan-out lands.
#[test]
#[ignore = "issue #426: multi-target-over-Player resolution fan-out not yet implemented \
            — Discard resolves only the first TargetRef::Player"]
...
///
/// KNOWN GAP (issue #426 — real root cause): with zero `TargetRef::Player`
/// targets, `resolve_player_for_context_ref`'s `find_map` finds nothing and
/// falls back to `ability.controller`, so the trigger's controller wrongly
/// discards a card. The per-targeted-player fan-out must treat an explicit
/// `TargetFilter::Player` with zero chosen players as a no-op (never fall
/// back to the controller). `#[ignore]` until the fan-out lands.
#[test]
#[ignore = "issue #426: zero-target multi-target-over-Player wrongly falls back to \
            ability.controller for the discard"]
